...
export function SearchCharResult({ searchResults }) {
                ...
                <div key={index} className="border-[1px] px-2 py-1 border-[#7e33ff89] rounded-sm relative">
                    ...
                    <p className="text-roboto font-normal text-base text-black">
                        ...
                        {result.sentence.word.map((word, index) => (
                            <span key={word._id}>
                            {index > 0 && " "}
                            {word._form.split('').map((letter, idx) => {
                                const wordLower = word._form.toLowerCase();
                                if (wordLower.includes(searchTerm)) {
                                    const startIndex = wordLower.indexOf(searchTerm);
                                    const endIndex = startIndex + searchTerm.length;
                                    const isWithinRange = idx >= startIndex && idx < endIndex;
                                    if (isWithinRange) {
                                        return (
                                            <span key={idx} style={{ color: '#7D33FF' }}>
                                                {letter}
                                            </span>
                                        );
                                    }
                            ...
                            <>
                                    ...
                                    <div className="bg-[#ffffff] w-[90%] h-[90%] rounded fixed left-1/2 top-6 -translate-x-1/2 overflow-y-scroll z-40 shadow-sm shadow-white scroll-bar">
                                        ...
                                        <div className="px-2 mt-3">
                                            ...
                                            {result.sentence.word.filter(word => word._form.toLowerCase().includes(searchTerm)).map((word, index) => (
                                                <div key={index}>
